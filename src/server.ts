...
export function getConfig(): SapConfig {
  ...
  const port = process.env.MCP_PORT || '8123'; // Default to 8123 if not set
...
- SAP_CLIENT`);
  ...
  return { url, username, password, client, port };
}

/**
 * Server class for interacting with ABAP systems via ADT.
 */
export class ABAPAdtMcpServer {
  ...
  /**
   * Starts the MCP server and connects it to the provided transport.
   * @param transport The StreamableHTTPServerTransport instance to connect to.
   */
  public async connectTransport(transport: StreamableHTTPServerTransport) {
...
const server = new ABAPAdtMcpServer();
